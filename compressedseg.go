...
func NewVolSize(x uint, y uint, z uint) VolSize {
	return VolSize{x, y, z}
}

// Compress64 takes an input 3D label array of a given volume dimensions and compresses
...
func Compress64(input []uint64, volume_size VolSize) (CompressedSegData, error) {
	BLKSIZE := uint(8)
...
// Decompress64 takes an input compressed array of 64-bit labels of a given volume dimensions
// and decompresses it. VolSize must be tileable by 8x8x8 block size.  The output
// data is an array of 64 bit numbers.
func Decompress64(input CompressedSegData, volume_size VolSize) ([]uint64, error) {
	BLKSIZE := uint(8)
			...
			for block[0] = 0; block[0] < grid_size[0]; block[0]++ {
				...
				for z := zmin; z < zmax; z++ {
					for y := ymin; y < ymax; y++ {
						...
						outindex := (z*(volume_size[1])+y)*volume_size[0] + xmin
